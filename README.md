...
- побудувати й інтерактивно розгорнути граф `документ → розділ → сегмент`;
...
Після сегментації кнопка **Build documentation graph** створює окремий
версіонований результат у таблиці `documentation_graph_results`. На початку UI
показує лише документи. Подвійний клік по документу розгортає його розділи,
а подвійний клік по розділу — найнижчі активні сегменти. Повторний подвійний
клік згортає відповідну гілку.
...
JSON графа містить статус `semantic_analysis: not_started`. Наступний етап має
додати LLM-аналізатор, який перетворюватиме сегменти на дії, умови та переходи.
Кожен його висновок повинен містити `evidence` з ідентифікатором вихідного
сегмента, короткою цитатою, оцінкою впевненості та версією моделі/промпту.
